...
class OpenAICompatEngine:
    ...
    def build_system_prompt(
        ...
        if self._external_system_prompt:
            ...
            "【字幕标点符号规范——严格遵守】\n"
            "1. **句末不加句号**：这是字幕场景，每行都是独立的时间轴片段，句末不需要句号（。）。\n"
            "2. **省略号处理**：\n"
            "   - 句子开头或句末的省略号（……）非必要不添加，原文没有则译文也不要加。\n"
            "   - 句子中间的省略号可以用逗号（，）代替，使译文更简洁。\n"
            "   - 仅当原文明确表示犹豫、拖音、未完成等语气时，才保留省略号。\n"
            "3. **破折号处理**：破折号（——）非必要不添加，仅在原文有明确的转折、打断、强调时才使用。\n"
            "4. **示例**：\n"
            "   - 原文「……今日も来たのね」→ 译文「今天也来了啊」（不加省略号、不加句号）\n"
            "   - 原文「それより、お茶でもいかが?」→ 译文「话说，要不要喝杯茶」（问号可保留）\n"
            "   - 原文「やめて……」→ 译文「住手」（不加省略号）\n"
            "   - 原文「なにこれ……舌使いだ」→ 译文「这是什么，舌头好厉害」（省略号改逗号）\n"
        )
