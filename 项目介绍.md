...
> 面向日语音声作品（RJ 音声 / ASMR）的**端到端字幕翻译引擎**：以官方台本为准绳，把「转录 → 对齐 → 翻译」做成一条全自动流水线。
...
日文音声作品要出中文字幕，通常卡在两件事上：

1. **转录天然不准**：耳语、喘息、气声、BGM、方言叠加，ASR 错误率很高，直接翻译等于在错误之上再叠一层错误。
2. **术语前后不一**：同一部作品里角色名、称呼、身体部位、设定用语反复变化，观感割裂。

HDEG 的做法是：**不跟 ASR 死磕，直接读官方台本**。台本随作品发售，是台词的权威原文。引擎把台本作为 Ground Truth，用来校正 ASR、统一术语、约束翻译风格。
...
- 支持 GPU/CPU，RTX 50 系自动切换精度。

### 2. 台本提取、识别与分割
- TXT 自动探测编码（UTF-8 / Shift-JIS / cp932）；PDF 直接提取。
- PDF **自动识别横排/竖排并重排**，解决日文竖排台本行序错乱。
- LLM 识别「哪个文件是台本」（从 readme / クレジット / 特典 中挑出真台本），判断是否已按音轨预分割；失败回退正则。
- 整本台本按音轨切分时采用**两级定位 + 回退链**：本地 FZ 锚点粗定位 → 低价模型在 ±50 行内精定位 → 正则兜底；预分割台本零 LLM 调用。
...
- 本地一致性检查器（零 LLM）找出译法不一致、术语未使用等问题。

### 5. 翻译与润色
- ASR 纠错四步工作流：可信度审查 → 上下文修复 → 中文翻译 → 译文回检。
